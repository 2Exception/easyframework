...
public class ExecuteInters {
	 ...
	 * @return
	 */
	public static Object executeIntersStart(List<Class<?>> list,HttpRequest request, HttpResponse response) {
		Class<?> clss = null;
		try {
			for(Class<?> cls : list) {
				clss = cls;
				...
				Method method2 = cls.getDeclaredMethod("startRequest", new Class[] { HttpRequest.class, HttpResponse.class });
	 ...
	 * @return
	 */
	public static Object executeIntersEnd(List<Class<?>> list,HttpRequest request, HttpResponse response,Object objs) {
		Class<?> clss = null;
		try {
			...
			for(Class<?> cls : list) {
				...
				Method method2 = cls.getDeclaredMethod("endRequest", new Class[] { HttpRequest.class, HttpResponse.class, Object.class });
	 ...
	 * @return
	 */
	public static List<Class<?>> getInters(String uriEnd){
			...
			if(obj != null) {
				...
				for(int i = 0;i<array.size();i++) {
					...
					if(MatchUtil.isMatch(ins.getString("pattern"), uriEnd)){
						list.add(Class.forName(ins.getString("class")));
					}
				}
			}
			return list;
